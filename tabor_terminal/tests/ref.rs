...
use tabor_terminal::event::{Event, EventListener};
use tabor_terminal::grid::{Dimensions, Grid};
use tabor_terminal::index::{Column, Line};
use tabor_terminal::term::cell::Cell;
use tabor_terminal::term::test::TermSize;
use tabor_terminal::term::{Config, Term};
...
fn ref_test(dir: &Path) {
    let recording = read_u8(dir.join("tabor.recording"));
